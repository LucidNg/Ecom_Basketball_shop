...
export async function Card({ limit }: CardProps) {
...
export async function Card2({ limit }: Card2Props) {
  let products: Array<Product> = [];

  try {
    products = await FetchProductByCategory("shoes");
  } catch (error) {
    console.error('Error fetching products:', error);
  }
...
export async function Card3({ limit }: Card3Props) {
  let products: Array<Product> = [];

  try {
    products = await FetchProductByCategory("clothes");
  } catch (error) {
    console.error('Error fetching products:', error);
  }
